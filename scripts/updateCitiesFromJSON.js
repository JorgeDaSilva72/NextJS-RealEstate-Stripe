...
function normalizeCityName(cityName) {
  // Enlever les nombres qui sont parfois dans le JSON
...
async function updateSeedWithCities() {
  try {
    ...
    const jsonPath = path.join(__dirname, '..', 'africanCities.json');
    const jsonContent = fs.readFileSync(jsonPath, 'utf-8');
    const citiesData = JSON.parse(jsonContent);

    // Lire le fichier seed actuel
    const seedPath = path.join(__dirname, '..', 'prisma', 'seed', '01_reference.ts');
    ...
    for (const [countryName, cities] of Object.entries(citiesData)) {
      ...
      if (countryRegex.test(seedContent)) {
        ...
        const citiesArray = validCities.map((cityName, index) => {
          ...
          const latVariation = (Math.random() * 0.5 - 0.25);
          const lngVariation = (Math.random() * 0.5 - 0.25);
          return `          {
            name: { fr: "${cityName}", en: "${cityName}" },
            lat: ${(defaultCoords.lat + latVariation).toFixed(4)},
            lng: ${(defaultCoords.lng + lngVariation).toFixed(4)},
